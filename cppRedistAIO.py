...
def download_vcredist():
    ...
    if "Location" not in r.headers:
        fatal("TechPowerUp did not return a redirect. Cannot get download URL.")
...
def extract_zip(zip_path: Path):
    ...
    if out_dir.exists():
        warn(f"Output directory already exists: {out_dir}")
...
def run_vcredists(out_dir: Path):
    ...
    for exe in installers:
        ...
        info(f"Installing {exe.name}  (version={ver}, arch={arch})")
